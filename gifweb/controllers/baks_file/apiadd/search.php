...
class Search extends MY_Controller
{
	...
	public function index()
	{
		...
		$mc_key = sha1('glapp_search_list_xuanyaodang' . serialize($this->platform . $device_id . $keyword . $type));
		if($page == 1){
			//$this->cache->memcached->save($mc_key, time(), 180);
			$this->cache->redis->set($mc_key, time(), 180);
		}
		//获取时间戳
		$node_time = 0;
		//$rs_time = $this->cache->memcached->get( $mc_key );
	...
	public function index_review_remap()
	{
		...
		// 防止翻页数据重复问题，记录用户请求第一页时间戳
		$mc_key = sha1('glapp_search_list_xuanyaodang' . ENVIRONMENT . serialize($this->platform . $device_id . $keyword . $type));
		if($page == 1){
			//$this->cache->memcached->save($mc_key, time(), 180);
			$this->cache->redis->set($mc_key, time(), 180);
		}
