...
class Gl extends MY_Controller
{
	 ...
	 *
	 */
	public function list_info_backup()
	{
		...
		// mc缓存
		$mcKey = sha1('get_list_info_qihu' . ENVIRONMENT .$ClassId.'_'.$page.'_'.$count.'_'.$max_id);
...
//            echo "<pre>";print_r($returns);exit;
			...
			$this->cache->redis->set($mcKey, json_encode($data), 60*5);
			Util::echo_format_return(_SUCCESS_, $data?$data:array());
		}catch (Exception $e) {
	...
	public function list_info()
	{
		...
		if($data === false || empty($data)){
			...
			// $this->cache->redis->lPush("gl_ll_debug", $_SERVER['REQUEST_URI'] . ':' . ($microtime - $GLOBALS['global_time_begin']) . json_encode($log));
			Util::echo_format_return(_SUCCESS_, $data ? $data : array());
			die();
		}
		...
		try {
			...
			//-->处理 如果max_id出现相同的 那么从下一页抓取最新的补齐   end --------//
			$GLOBALS['list_info_yar_data'] = array();
			function callback($retval, $callinfo) {
				...
				if (is_array($retval) && isset($retval['sort'])) {
					$GLOBALS['list_info_yar_data'][$retval['sort']] = $retval['data'];
				}
			}
			$yarUrl = "http://www.wan68.com/yar/yar/controller/yar/gl";
			try {
				foreach ($returns as $k=>$v){
					$_arr = array(
						'_id' => $v['_id'],
					...
					Yar_Concurrent_Client::call($yarUrl, "list_info_detail", array($_arr, $k), "callback", NULL);
				}
			} catch (Yar_Client_Protocol_Exception $e) {
				throw new Exception('yar error', _PARAMS_ERROR_);
			}
			Yar_Concurrent_Client::loop(); //send
			Yar_Concurrent_Client::reset();

			$data = $GLOBALS['list_info_yar_data'];
			ksort($data);
			$this->cache->redis->set($mcKey, json_encode($data), 60*5);
	...
	public function list_info_yar()
	{
		...
		try {
			...
			$GLOBALS['list_info_yar'] = array();
			function callback($retval, $callinfo) {
				if ($callinfo == NULL) {
			       //做本地的逻辑
			       return TRUE;
			    }
				$GLOBALS['list_info_yar'] = array_merge($GLOBALS['list_info_yar'], $retval);
			}

	        $url = "http://www.wan68.com/yar/yar/controller/yar/gl";
			$c = 10;
			$begin = intval(($page - 1) * $count / $c) * $c;
			$end = intval(($page ) * $count / $c) * $c;

			$redun_shift = $page * $count - $begin;
			$redun_pop = $end - $page * $count;
			...
			try {
				while ($begin < $end) {
					...
					$begin += $c;
					Yar_Concurrent_Client::call($url, "list_info", $params, "callback", NULL);
				}
			} catch (Yar_Client_Protocol_Exception $e) {
				var_dump($e);
			}
	...
	public function detail_info()
	{
		...
		// mc缓存
		$mcKey = sha1('detail_info_qihu' . ENVIRONMENT .$newsid.'_'.$guid);
		...
		try {
			...
			if(1){ //360方面要求content内容自带视频
				//调用移动端处理model来获取数据
				$wap_result = $this->waptext_model->get_cms_info($newsid, $device);

				if($wap_result['msg'] == 1){ //获取成功
					$contents_360 = $wap_result['html'];
				}else{ //获取数据失败
					throw new Exception('数据不存在', _PARAMS_ERROR_);
				}
				$newsInfo["content_360"] = $contents_360;
			}

			$newsInfo["content_360"] = $this->waptext_model->clear_qr_code($newsInfo["content_360"]);
			...
			//echo $newsInfo["content_360"];exit;
			Util::echo_format_return(_SUCCESS_, $newsInfo);
		}catch (Exception $e) {
			Util::echo_format_return($e->getCode(), array(), $e->getMessage());
		}
	...
	public function juhe_page()
	{
		...
		try {
			...
			//$this->request_log("5 step: 获取游戏一级分类之后");
			...
			//$this->request_log("5 step: 获取游戏二级分类之后");
			//［三级分类］
			...
			//	$this->request_log("6 step: 获取游戏三级分类之后");
				foreach ($info_b as $k => &$v ){
			...
			//	$this->request_log("7 step: 遍历处理get_category_list三级");
			}
			...
			unset($raidersClassList['raiderCount']);
			$data  = $info_a;
			unset($data['id']);
...
// 			$data['focusList'] =array();
			...
			//====================炫耀党接口数据处理结束=======================
			//$this->request_log("9 step: success");
	...
	public function detail_info_review_remap()
	{
		...
		// mc缓存
		$mcKey = sha1('detail_info_' . ENVIRONMENT .$newsid.'_'.$guid . '_review1' );
